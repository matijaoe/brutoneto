...
export function calculateIncomeAfterDeductions(gross: number, pensionContribution: number): number {
  return gross - pensionContribution;
}

export function calculatePersonalAllowance(income: number, coefficient: number): number {
  return Math.min(income, BASIC_PERSONAL_ALLOWANCE * coefficient);
}

export function calculateTaxableIncome(income: number, personalAllowance: number): number {
  return income - personalAllowance;
}

export function calculateTax(principal: number, taxRateLow: number, taxRateHigh: number): number {
  const taxLowerBracket = Math.min(principal, HIGH_TAX_BRACKET_THREASHOLD) * taxRateLow;
  const taxHigherBracket = Math.max((principal - HIGH_TAX_BRACKET_THREASHOLD) * taxRateHigh, 0);
...
export function calculateNetSalary(
  ...
  const personalAllowance = calculatePersonalAllowance(income, personalAllowanceCoefficient);
...
export function brutoToBruto2(gross: number): number {
  const healthInsuranceContribution = calculateHealthInsuranceContribution(gross);
